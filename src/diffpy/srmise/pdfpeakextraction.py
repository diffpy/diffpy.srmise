...
class PDFPeakExtraction(PeakExtraction):
    """PDFPeakExtraction extends the PeakExtraction class to specialize
    in extracting peaks from PDF (Probability Density Function) data.
    ...
    def fit(self, **kwds):
        """Fit peaks in the PDF. Returns ModelCovariance instance
        summarizing results.
    ...
    def writepwastr(self, comments):
        """Return string of extracted peaks (position, width, area) in
        PDF.
...
def stdratio(data):
    """Calculate ratio of standard deviation for runs of equal length in
    data.
