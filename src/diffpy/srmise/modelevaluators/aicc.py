...
class AICc(ModelEvaluator):
    ...
    def evaluate(self, fit, count_fixed=False, kshift=0):
        """Return quality of fit for given ModelCluster using AICc
        (Akaike's Information Criterion with 2nd order correction for
        small sample size).
    ...
    def minpoints(self, npars):
        """Calculates the minimum number of points required to make an
        estimate of a model's quality.
    ...
    def growth_justified(self, fit, k_prime):
        """Is adding k_prime parameters to ModelCluster justified given
        the current quality of the fit.
